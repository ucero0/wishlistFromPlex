...
class DelugeClient:
    ...
    def probe_connection(self) -> None:
        """Raise when Deluge RPC is unreachable or authentication fails."""
        if self.is_connected:
            return
        if self.connect():
            return
        err = self.last_connect_error or "unknown error"
        ...
        raise DelugeConnectionError(f"Cannot connect to Deluge at {target}: {err}")
