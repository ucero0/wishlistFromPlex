...
prowlarr_routes = APIRouter(prefix="/prowlarr", tags=["prowlarr"])


@prowlarr_routes.post("/search/by-query", response_model=SearchResponse)
async def search_torrents_by_query(
    request: SearchByQueryRequest,
    find_query: GetBestTorrentsQuery = Depends(create_find_best_torrent_query),
...
@prowlarr_routes.get("/test-connection", response_model=ProwlarrConnectionResponse)
async def test_prowlarr_connection(
    ...
    return external_connection_to_json_response(status, version=status.version)


@prowlarr_routes.get("/indexers/count", response_model=ProwlarrIndexerCountResponse)
