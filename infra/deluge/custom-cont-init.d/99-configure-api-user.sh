...
# Apply Deluge RPC + Web UI credentials from .env on first start (and keep RPC in sync).
set -euo pipefail
...
if not wait_for(AUTH_FILE):
    print("[deluge-init] /config/auth not found yet, skipping credential setup")
    raise SystemExit(0)
...
if not wait_for(WEB_FILE, attempts=15, delay=2.0):
    print("[deluge-init] /config/web.conf not ready yet; RPC is configured. Web UI password stays default until next restart.")
    raise SystemExit(0)
